  ...
  return (
    <Container className="profile2Form">
      ...
      <Row>
        ...
        <Col>
          {currentProfile === "" && user.position === "customer" ? (<Button onClick={updateCustomer} variant="dark">Customer</Button>)
          : (<Button onClick={updateCustomer} variant={customer}>Customer</Button>)}
        </Col>
        <Col>
          {currentProfile === "" && user.position === "driver" ? (<Button onClick={updateDriver} variant="dark">Driver</Button>)
          : (<Button onClick={updateDriver} variant={driver}>Driver</Button>)}
        </Col>
        {/* onClick replaces default view with User's selection */}
        {currentProfile === "" ? (<Row>{defaultProfile}</Row>) : (<Row>{currentProfile}</Row>)}
      </Row>
    </Container>
  );
};
